...
const schema = z
  .object({
    ...
    contact: z.string().min(6, "Téléphone ou email requis"),
    truckTypes: z
      .array(
        z.object({
  ...
  .superRefine((data, ctx) => {
    ...
    if (!isEmail && !/^[\d\s+()-]{6,32}$/.test(data.contact.trim())) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Saisissez un téléphone ou une adresse email valide",
        path: ["contact"],
      });
    }
  });
      ...
      <CardContent className="p-4 sm:p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <Input
              type="text"
              placeholder="+229 00 00 00 00 ou contact@exemple.com"
              {...register("contact")}
            />
            {errors.contact && (
          ...
          <div className="space-y-1">
            <label className="text-sm font-medium text-foreground">
              Espace pour laisser un message spécifique
            </label>
            <Textarea
              rows={4}
              placeholder="Précisez vos disponibilités, zone d'activité..."
              {...register("message")}
            />
            {errors.message && (
          ...
          <Button
            ...
            size="lg"
          >
            {isSubmitting ? "Envoi en cours..." : "Référencer mon camion"}
          </Button>
        </form>
      </CardContent>
    </Card>
