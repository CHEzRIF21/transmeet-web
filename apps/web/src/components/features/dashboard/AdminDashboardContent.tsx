...
import { useAdminStats, useAdminMissions } from "@/lib/api/dashboard";
...
const KPI_CONFIG = [
  { key: "missions" as const, label: "Missions totales", icon: ClipboardList, iconBg: "bg-primary/10", iconColor: "text-primary" },
  { key: "utilisateurs" as const, label: "Utilisateurs", icon: Users, iconBg: "bg-green-100", iconColor: "text-green-700" },
  { key: "demandes" as const, label: "Demandes en attente", icon: FileText, iconBg: "bg-accent/15", iconColor: "text-accent" },
  { key: "revenus" as const, label: "Revenus plateforme", icon: TrendingUp, iconBg: "bg-yellow-100", iconColor: "text-yellow-600" },
];

export function AdminDashboardContent() {
  const { data: stats, isLoading: statsLoading } = useAdminStats();
  const { data: missions = [], isLoading: missionsLoading } = useAdminMissions();
  ...
  const kpiValues = {
    missions: stats?.missionsTotal ?? 0,
    utilisateurs: stats?.utilisateursTotal ?? 0,
    demandes: stats?.demandesEnAttente ?? 0,
    revenus: stats?.revenusPlateforme ?? 0,
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Vue d&apos;ensemble de la plateforme Transmeet.
          </p>
        </div>
      </div>
        ...
        <CardHeader className="flex flex-row items-center justify-between px-5 pb-3 pt-5">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {missions.length} dernières missions
            </p>
          </div>
          <Button variant="outline" size="sm" asChild>
            <Link href="/admin/missions" className="gap-1">
              Voir tout
