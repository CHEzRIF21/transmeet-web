...
import { useTransporteurStats, useTransporteurMissions } from "@/lib/api/dashboard";
...
const KPI_CONFIG = [
  ...
  { key: "vehicules" as const, label: "Véhicules actifs", icon: Layers, iconBg: "bg-accent/15", iconColor: "text-accent" },
  { key: "livrees" as const, label: "Missions livrées", icon: ClipboardCheck, iconBg: "bg-yellow-100", iconColor: "text-yellow-600" },
];

interface TransporteurDashboardContentProps {
  userId: string;
}

export function TransporteurDashboardContent({ userId }: TransporteurDashboardContentProps) {
  const { data: stats, isLoading: statsLoading } = useTransporteurStats(userId);
  const { data: missions = [], isLoading: missionsLoading } = useTransporteurMissions(userId);
  ...
  const kpiValues = {
    ...
    vehicules: stats?.vehiculesActifs ?? 0,
    livrees: stats?.missionsLivrees ?? 0,
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Bienvenue — gérez vos véhicules et missions.
          </p>
        </div>
        <Button variant="accent" size="default" className="gap-2" asChild>
          <Link href="/transporteur/vehicules">
      ...
      <Card className="border-primary/10">
        <CardHeader className="flex flex-row items-center justify-between px-5 pb-3 pt-5">
          ...
          <Button variant="outline" size="sm" asChild>
            <Link href="/transporteur/missions" className="gap-1">
              Voir tout
