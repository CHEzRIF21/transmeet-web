...
const sendBodySchema = zod_1.z.object({
    ...
    missionId: zod_1.z.string().uuid().optional(),
    expediteurId: zod_1.z.string().uuid(),
    transporteurId: zod_1.z.string().uuid(),
    content: zod_1.z.string().min(1),
    type: zod_1.z.enum(["TEXT", "FILE", "OFFER", "SYSTEM"]).optional(),
    fileUrl: zod_1.z.string().url().optional(),
});
async function messagesRoutes(app, _opts) {
    ...
    app.get("/conversations", async (request, reply) => {
        ...
        const withLastMessage = await Promise.all(conversations.map(async (c) => {
            const last = await prisma_js_1.prisma.message.findFirst({
                where: { conversationId: c.id },
                orderBy: { sentAt: "desc" },
                select: { content: true, sentAt: true },
            });
            const unread = await prisma_js_1.prisma.message.count({
                where: {
                    conversationId: c.id,
                    senderId: { not: userId },
                    readAt: null,
    ...
    app.get("/", async (request, reply) => {
        ...
        const userId = request.user.id;
        const conv = await prisma_js_1.prisma.conversation.findFirst({
            where: {
                id: conversationId,
                OR: [{ expediteurId: userId }, { transporteurId: userId }],
            },
    ...
    app.post("/", async (request, reply) => {
        ...
        const userId = request.user.id;
        const { conversationId, missionId, expediteurId, transporteurId, content, type, fileUrl } = parsed.data;
        let convId = conversationId;
        if (!convId) {
            const existing = await prisma_js_1.prisma.conversation.findFirst({
                where: {
            ...
            if (existing) {
                ...
                const created = await prisma_js_1.prisma.conversation.create({
                    data: {
                        missionId,
                        expediteurId,
                        transporteurId,
